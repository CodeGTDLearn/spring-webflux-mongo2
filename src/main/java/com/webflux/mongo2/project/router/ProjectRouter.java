...
import static com.webflux.mongo2.config.Routes.*;
...
public class ProjectRouter {
  ...
  public RouterFunction<ServerResponse> routeProjects(ProjectHandler handler) {
    ...
    return RouterFunctions
         ...
         .andRoute(POST(TASK_CREATE).and(accept(JSON)),handler::createTask)
         ...
         .andRoute(GET(PROJ_COST_GREATER_THAN).and(accept(JSON)),handler::findByEstimCostGreatThan)
         ...
         .andRoute(GET(PROJ_QUERY_BYNAME_COST).and(accept(JSON)),handler::findByNameAndCostQuery)
         .andRoute(GET(PROJ_QUERY_EST_COST_BET).and(accept(JSON)),handler::findByEstCostBetwQuery)
         .andRoute(GET(PROJ_QUERY_NYNAME_REGEX).and(accept(JSON)),handler::findByNameRegexQuery)
         .andRoute(GET(PROJ_TEMPL_BYNAME_QUERY).and(accept(JSON)),handler::findProjByNameQueryTempl)
         ...
         .andRoute(POST(PROJ_TEMPL_UPCOST_CRIT).and(accept(JSON)),handler::upsertCostWithCritTempl)
         .andRoute(DELETE(PROJ_TEMPL_DEL_CRIT).and(accept(JSON)),handler::deleteCriteriaTempl)
